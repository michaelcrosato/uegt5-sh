...
| 30 | 2026-08-31 | **The flash-check audit** (`-fcflashcheck` + `-fctour`: every light killed, torch forced on, stations captured from the player's eyes) found and fixed three real renderer bugs and drove the survival-horror relight: (a) the hidden shadow proxy fully ECLIPSED the flashlight's surface lighting — only the volumetric cone ever showed; the proxy now stops casting while the torch is on; (b) `AutoExposureMaxBrightness 0.18` clipped any bright surface to featureless white — raised to 1.5 (darkness is guarded by MIN, which is unchanged); (c) the beam was untinted studio white with a plateau cone — now warm (1.0/0.88/0.72), inner 7° / outer 31°. Interiors relit: bulbs dimmed to 10–22cd, corner-placed, palette-varied (amber / dingy white / sickly green / cold blue), deterministic hum/gutter flicker; interact prompt now mint-cyan on a black drop shadow (was white-on-glare). M1 walk window widened +120 frames (~280fps runs outpaced a 170cm stride); Shell FOV check made clamp-proof. Full regression green | Playtest asks: prompt legibility, "the first building is the most well lit building", and "does the flashlight look state of the art?" — it could not, because it was eclipsed; the audit harness is now a standing tool |
| 31 | 2026-08-31 | **Torch de-glare pass** (playtest: "dustiest air… blinding white light") — every number that had been cranked to compensate for the eclipse era came back down now that surfaces light: volumetric scatter 8→0.05 (the beam reads by what it LIGHTS; the air carries a trace), intensity 1700→1000cd, cones 7/31→6/22 (no more ground-splash at your feet), bloom 0.675→0.4 in all scene PP volumes. Interact prompt now sits on an opaque black plate (a drop shadow alone washed out against the disc). New `-fcflashon` harness flag: torch forced on with world lights KEPT — the normal-play condition the complaint came from, now capturable. Verified in both conditions; M1/M4/Shell green | Second-order lesson from #30: fixing a masking bug re-tunes everything that was calibrated against the mask |
